...
v^^>>><<^^<>>^v^<v^vv<>v^<<>^<^v^v><^<<<><<^<v><v<>vv>>v><v^<vv<>v^<<^
    ...
    val (rawMap, rawMovements) = File("inputs/day15.txt").readText().trim().split("\n\n")
    ...
    val movements = rawMovements.mapNotNull {
        when (it) {
            '<' -> Direction.LEFT
            '^' -> Direction.UP
            '>' -> Direction.RIGHT
...
private fun part1(map: List<List<Tile>>, movements: List<Direction>): Long {
    ...
    return mapCopy.withIndex().sumOf { (y, line) ->
        line.withIndex().sumOf { (x, tile) ->
            if (tile == Tile.BOX) x + 100L * y
            else 0L
        }
    }
}

private fun expandMap(map: List<List<Tile>>): List<List<WideTile>> =
    map.map { line ->
        line.flatMap { tile ->
            when (tile) {
...
private fun part2(map: List<List<Tile>>, movements: List<Direction>): Long {
    ...
    return mapCopy.withIndex().sumOf { (y, line) ->
        line.withIndex().sumOf { (x, tile) ->
            if (tile == WideTile.BOX_LEFT) x + 100L * y
            else 0L
        }
    }
}
