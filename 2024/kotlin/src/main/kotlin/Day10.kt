...
private fun findTrails(startPos: Vec2, map: List<List<Int>>): List<Vec2> {
    ...
    while (queue.isNotEmpty()) {
        ...
        if (pos.isInBounds(map) && map[pos] == height + 1) {
            if (height == 8) targets.add(pos)
            else Direction.entries.map { it.move(pos) to height + 1 }.forEach { queue.add(it) }
        }
    }
    return targets
}
...
private fun part1(input: List<List<Int>>): Int =
    sumTrails(input, distinctTargets = true)

private fun part2(input: List<List<Int>>): Int =
    sumTrails(input, distinctTargets = false)
