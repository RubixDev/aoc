...
fun day24(input: String): Day = sequence {
    val example = """
    x00: 1
    x01: 0
    x02: 1
    ...
    val (rawInputs, rawGates) = input.split("\n\n")
    val inputs = rawInputs.lines()
        .map { line -> line.split(": ") }
        .associate { (wire, value) -> wire to value.toInt().toBoolean() }
    val gates = rawGates.lines()
        .map { line -> line.split(" -> ") }
        .associate { (calc, output) ->
            output to
...
private fun part1(
    ...
    gates: Map<String, Pair<String, Set<String>>>,
): Long {
...
private fun part2(
    ...
    for ((gate, compute) in ors) {
        ...
        if (gate.startsWith('z') && gate != "z45") {
            println("WRONG:::: incorrect OR (direct output to z) ::: $gate").also {
                wrong.add(gate)
            }
        }
    ...
    for ((gate, compute) in xors) {
        ...
        if (gates[new]!!.first != "XOR") {
            println("WRONG:::: incorrect XOR (wrong op for new bit)::: $new").also {
                wrong.add(new)
            }
        }
    }
    for ((gate, compute) in ands) {
        ...
        if (gate.startsWith('z')) {
            println("WRONG:::: incorrect AND (direct output to z) ::: $gate").also {
                wrong.add(gate)
            }
        }
        if (ins.any { it.startsWith('x') }) continue
