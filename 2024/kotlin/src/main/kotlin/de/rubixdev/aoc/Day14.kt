...
fun day14(input: String): Day = sequence {
...
private fun part1(input: List<Pair<Vec2, Vec2>>): Int {
    ...
    return robotCounts.entries.filter {
        it.key.x < SIZE.x / 2 && it.key.y < SIZE.y / 2
    }.sumOf { it.value } *
        robotCounts.entries.filter {
            it.key.x > SIZE.x / 2 && it.key.y < SIZE.y / 2
        }.sumOf { it.value } *
        robotCounts.entries.filter {
            it.key.x < SIZE.x / 2 && it.key.y > SIZE.y / 2
