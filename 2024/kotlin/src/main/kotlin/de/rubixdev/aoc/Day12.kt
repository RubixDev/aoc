    ...
    return input.withIndex().sumOf { (y, line) ->
        line.withIndex().sumOf { (x, _) ->
            ...
            floodRegion(x by y, input, visited) { search, neighbours ->
                region[search] =
                    Direction.entries.filter { dir -> neighbours.none { it.first == dir } }
            }
            if (region.isEmpty()) {
                0
            } else {
                ...
                while (region.entries.any {
                        it.value.isNotEmpty() &&
                            it.value.any { d -> (it.key to d) !in border }
                    }
                ) {
                    ...
                    var first = true
                    do {
                        while (region[pos + direction.rotateRight().vec]?.contains(direction) ==
                            true
                        ) {
